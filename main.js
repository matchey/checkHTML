					...
					return function (statusReason) {
						return function (userRegister) {
							return function (user_info) {
								return function (isMenuOpen) {
									return function (navbarState) {
										return {inputCred: inputCred, isMenuOpen: isMenuOpen, key: key, loginStatus: loginStatus, navbarState: navbarState, statusReason: statusReason, tokenUserId: tokenUserId, url: url, userRegister: userRegister, user_info: user_info};
									};
								};
							};
						};
...
var $author$project$Main$init = F3(
	function (flags, url, key) {
		...
		var navbarCmd = _v0.b;
		return _Utils_Tuple2(
			$author$project$Main$Model(key)(url)($author$project$Main$initCred)($elm$core$Maybe$Nothing)($author$project$Main$LoginOff)('')(false)(
				_List_fromArray(
					[$author$project$Main$initUserInfo]))(false)(navbarState),
...
var $author$project$Main$errToString = function (err) {
	switch (err.$) {
		...
		default:
			var status = err.a;
			return (status === 401) ? 'User ID or Password is incorrect' : ('BadStatus : ' + $elm$core$String$fromInt(status));
	}
};
...
var $author$project$Main$registerUrl = '/user_info';
var $author$project$Main$serverUrl = 'http://localhost:8821';
var $author$project$Main$submitCredentials = function (model) {
	var url = _Utils_ap($author$project$Main$serverUrl, $author$project$Main$registerUrl);
	...
	return $elm$http$Http$request(
		{
			...
			url: A3(
				...
				_List_fromArray(
					['user_info', model.inputCred.user_id]),
				params)
		});
};
		...
		switch (msg.$) {
			...
			case 'Register':
				if ((model.inputCred.password === '') || (model.inputCred.user_id === '')) {
					...
					var _v4 = _Utils_eq(model.inputCred.password, model.inputCred.password2);
					if (_v4) {
						return _Utils_Tuple2(
							model,
			...
			case 'ResToken':
				...
				var token = _v8.a;
				var name = _v8.b;
				var _v9 = ($elm$core$String$length(token) > 0) && ($elm$core$String$length(name) > 0);
				if (!_v9) {
					return _Utils_Tuple2(
						_Utils_update(
							model,
																			...
																			[
																				...
																				_List_fromArray(
																					[
																						...
																						_List_fromArray(
																							[
																								$elm$html$Html$Attributes$placeholder('Nickname (optional)')
																							])),
